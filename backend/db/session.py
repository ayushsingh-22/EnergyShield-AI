...
def session_scope() -> Iterator[Session | None]:
    ...
    try:
        ...
        if not _unavailable_warned:
            logger.warning("Could not open a database session; persistence calls will be skipped.")
    ...
    try:
        ...
        if not _unavailable_warned:
            logger.warning(
                "Database write failed; persistence calls will be skipped until it recovers.", exc_info=True
            )
            _unavailable_warned = True
    finally:
        session.close()
